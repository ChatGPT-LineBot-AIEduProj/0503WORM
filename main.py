...
model_management = {}
api_keys = {}
api_key = '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'  #直接在這裡改
  ...
  #調用答案
    ...
    if 'A' == stu_nowq_dic['a']:
      ...
      if count_FQnum_list == 0:
        score = '太好了!第一次就答對了!(+3分)'
      elif count_FQnum_list <= 2:
        score = '訂正後答對了!(+2分)'
      else:
        score = '答對了!(+1分)'
      msg = TextSendMessage(text=score)
      stuResp(user_id, time, score, "(系統)")
      ...
      if count_FQnum_list <= 2:
        ...
        score = '或許你可以尋求幫助...' + '(' + str(stu_nowq_dic['tip']) + ')'
      else:
        score = '別灰心!訂正好後再做答吧!' + '(' + str(stu_nowq_dic['explain']) + ')'
      msg = TextSendMessage(text=score)
      stuResp(user_id, time, score, "(系統)")
